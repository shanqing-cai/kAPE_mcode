...
% Last Modified by GUIDE v2.5 15-Aug-2012 18:30:19
...
%             t_hsp = subplot('Position', [handles.leftMargin + (i1 - 1) * (handles.panelWidth + handles.panelSpacing), 0.375, handles.panelWidth, handles.panelHeight]);
            ...
            impostfix = {'', '', ''};
            if uiConfig.bShowCorrAnim == 0 && uiConfig.bVaryColor == 1
                for kk = 1 : 3
                    if rand > 0.5
                        impostfix{kk} = '_b';
                    else
            ...
            if i1 == 1
                im_trg = handles.skin.(['bed', impostfix{1}]);
                t_word = 'bed';
            elseif i1 == 2
                if uiConfig.bShowCorrAnim == 1
                    im_trg = handles.skin.(['head', impostfix{2}]);
                else
                    im_trg = handles.skin.(['head0', impostfix{2}]);
                end
                t_word = 'head';
            elseif i1 == 3
                im_trg = handles.skin.(['Ted', impostfix{3}]);
                t_word = 'Ted';
            end
...
if isequal(str, 'Start pitch shifter')
    set(handles.pb_pitchShift, 'String', 'Stop pitch shifter');
    doPitchShift(pst);
    
else
    TransShiftMex_PS(2);
    set(handles.pb_pitchShift, 'String', 'Start pitch shifter');
end
return
