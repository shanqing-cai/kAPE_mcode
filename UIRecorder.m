...
% Last Modified by GUIDE v2.5 17-Aug-2012 14:50:27
...
handles.bShowOnlyIncorrect = 0;
handles.bVaryColor = 1;
handles.bShowReadyGo = 1;
...
skin.bed_b = double(imread(fullfile(pwd, 'images', 'cartoon-bed-1b.png'))) / 255.;
...
skin.head0_b = double(imread(fullfile(pwd, 'images', 'cartoon-head-1b.png'))) / 255.;
...
skin.Ted_b = double(imread(fullfile(pwd, 'images', 'cartoon-ted-1b.png'))) / 255.;
skin.Ted2 = double(imread(fullfile(pwd, 'images', 'cartoon-ted-2.png'))) / 255.;

skin.cat = double(imread(fullfile(pwd, 'images', 'cartoon-cat-1.jpg'))) / 255.;
skin.pig = double(imread(fullfile(pwd, 'images', 'cartoon-pig-1.jpg'))) / 255.;
...
set(gca, 'XTick', [], 'YTick', []);
set(gca, 'XColor', 'w', 'YColor', 'w');
htxt_corrCnt = text(0, 0, ...
                    sprintf('You have completed %d of %d parts!', 0, numel(handles.allPhases)), ...
                    'Color', 'b', 'FontSize', 14);
...
if ~isfile(handles.uiConfigFN)
    uiConfig = struct('showWordHint', handles.showWordHint, ...
                      ...
                      'bShowReadyGo', handles.bShowReadyGo, ...
                      'bShowOnlyIncorrect', handles.bShowOnlyIncorrect, ...
    ...
    handles.bShowReadyGo = uiConfig.bShowReadyGo;
    handles.bShowOnlyIncorrect = uiConfig.bShowOnlyIncorrect;
    handles.bVaryColor = uiConfig.bVaryColor;
...
if (handles.debug==0)
    ...
    %     end
        idxPhase = strmatch(handles.phase, handles.allPhases, 'exact');
        if isempty(idxPhase)
            idxPhase = numel(handles.allPhases);
        end

        newTxt = sprintf('You have completed %d of %d parts!', ...
                         idxPhase - 1, numel(handles.allPhases));
    ...
    if (handles.trialType==1 || handles.trialType==2)
        ...
        a_alternWords = {'Ted', 'cat', 'pig'};
        a_regularWords = {'bed', 'head', 'Ted'};
        
        if ~isequal(handles.phase, 'other')
            t_words = a_regularWords;
        else
            t_words = a_alternWords;
        end
...
%             cla;
            ...
            set(gcf, 'CurrentAxes', handles.hsp(spOrder(i1)));
...
%             t_hsp = subplot('Position', [handles.leftMargin + (i1 - 1) * (handles.panelWidth + handles.panelSpacing), 0.375, handles.panelWidth, handles.panelHeight]);
            ...
            impostfix = {'', '', ''};
            if uiConfig.bShowCorrAnim == 0 && uiConfig.bVaryColor == 1 && ~isequal(handles.phase, 'other')
                for kk = 1 : 3
                    if rand > 0.5
                        impostfix{kk} = '_b';
                    else
            ...
            if i1 == 1
                im_trg = handles.skin.([t_words{1}, impostfix{1}]);
                t_word = t_words{1};
            elseif i1 == 2
                if uiConfig.bShowCorrAnim == 1
                    im_trg = handles.skin.([t_words{2}, impostfix{2}]);
                else
                    if ~isequal(handles.phase, 'other')
                        im_trg = handles.skin.([t_words{2}, '0', impostfix{2}]);
                    else
                        im_trg = handles.skin.([t_words{2}, impostfix{2}]);
                    end
                end
                t_word = t_words{2};
            elseif i1 == 3
                im_trg = handles.skin.([t_words{3}, impostfix{3}]);
                t_word = t_words{3};
            end
        ...
        if uiConfig.bShowReadyGo
            if ~uiConfig.bShowOnlyIncorrect
                for flashCnt = 1 : 1
                    htxt = text(0, 0, 'Ready...', 'FontSize', 24, 'Color', [0.5, 0.5, 0]);
                    drawnow;
                    pause(0.75);
                    delete(htxt);
                    drawnow;
                ...
                htxt = text(0, 0, 'Go!', 'FontSize', 24, 'Color', [0, 0.5, 0]);
                pause(0.75);
                delete(htxt);
...
%         a_words = {'bed', 'head', 'ted'};
        for idx = 1 : numel(t_words)
            if isequal(lower(t_words{idx}), lower(handles.word));
                break;
            end
        end
%         idx = fsic(t_words, lower(handles.word));
        set(gcf, 'CurrentAxes', handles.hsp(spOrder(idx)));
%         cla;
...
%         end
        ...
        load(handles.uiConfigFN);
        handles.showWordHint = uiConfig.showWordHint;
        if handles.showWordHint
            xs = get(gca, 'XLim');
            ys = get(gca, 'YLim');
            if isequal(lower(handles.word), 'bed')
                text(xs(1) + 0.2 * range(xs), ys(1) + 0.92 * range(ys), handles.word, ...
                     'FontSize', 24, 'FontWeight', 'bold', 'Color', 'b');
            elseif isequal(lower(handles.word), 'head')
                if uiConfig.bShowCorrAnim
                    text(xs(1) + 0.34 * range(xs), ys(1) + 0.95 * range(ys), handles.word, ...
                         'FontSize', 24, 'FontWeight', 'bold', 'Color', 'b');
                else
                    text(xs(1) + 0.04 * range(xs), ys(1) + 0.95 * range(ys), handles.word, ...
                         'FontSize', 24, 'FontWeight', 'bold', 'Color', 'b');
                end
            elseif isequal(lower(handles.word), 'pig')
                text(xs(1) + 0.6 * range(xs), ys(1) + 0.92 * range(ys), handles.word, ...
                     'FontSize', 21, 'FontWeight', 'bold', 'Color', 'b');
            elseif isequal(lower(handles.word), 'cat')
                text(xs(1) + 0.84 * range(xs), ys(1) + 0.92 * range(ys), handles.word, ...
                     'FontSize', 21, 'FontWeight', 'bold', 'Color', 'b');
            else
                text(xs(1) + 0.42 * range(xs), ys(1) + 0.92 * range(ys), handles.word, ...
                     'FontSize', 21, 'FontWeight', 'bold', 'Color', 'b');
            end
        end
        ...
        if uiConfig.trialStartWithAnim == 1
            ...
            if uiConfig.trialPresetDur == 1
                TransShiftMex(3, 'triallen', handles.trialLen);
            else
                TransShiftMex(3, 'triallen', handles.trialLenMax);
            end
...
%         tic;
        if uiConfig.trialStartWithAnim == 0
            if handles.bAO==1
                MexIO('reset');
                TransShiftMex(3, 'scale', handles.dScale);
            else            
                MexIO('reset');
                if uiConfig.trialPresetDur == 1
                    TransShiftMex(3, 'triallen', handles.trialLen);
...
%         fprintf('Start-up time = %.3f sec\n',startupTime);
    ...
    set(handles.button_markBad, 'enable', 'on');
...
%     durRes = 1;   % 0 - good; -1 - too shoft; +1 - too long
    ...
    if resp_correct == 1
        if isequal(handles.phase, 'pre')
            if uiConfig.bShowCorrAnim
                show_corr_anim(handles, spOrder);
            end
        elseif isequal(handles.phase, 'pract1')
            if rmsRes == 0
                if uiConfig.bShowCorrAnim
            ...
            if rmsRes == 0 && durRes == 0
                if uiConfig.bShowCorrAnim  && ~isequal(handles.phase, 'other')
                    show_corr_anim(handles, spOrder);
                end
            end
        end        
    ...
    % Marked as bad?
    if isequal(get(handles.button_markBad, 'String'), 'Mark as bad')
        dataOut.exptMarkBad = 0;
    else
        dataOut.exptMarkBad = 1;
    end
...
% --- Executes on button press in rb_showOnlyIncorrect.
function rb_showOnlyIncorrect_Callback(hObject, eventdata, handles)
% hObject    handle to rb_showOnlyIncorrect (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of rb_showOnlyIncorrect
val = get(handles.rb_showOnlyIncorrect, 'Value');
load(handles.uiConfigFN);   % gives uiConfig
uiConfig.bShowOnlyIncorrect = val;
save(handles.uiConfigFN, 'uiConfig');
...
% --- Executes on button press in rb_varyColor.
function rb_varyColor_Callback(hObject, eventdata, handles)
% hObject    handle to rb_varyColor (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of rb_varyColor
val = get(handles.rb_varyColor, 'Value');
load(handles.uiConfigFN);   % gives uiConfig
uiConfig.bVaryColor = val;
save(handles.uiConfigFN, 'uiConfig');

return


% --- Executes on button press in pb_pitchShift.
function pb_pitchShift_Callback(hObject, eventdata, handles)
% hObject    handle to pb_pitchShift (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
str = get(handles.pb_pitchShift, 'String');
pst = str2num(get(handles.edit_pitchShift, 'String'));

if isempty(pst) || isnan(pst)
    pst = 3;
    set(handles.edit_pitchShift, 'String', sprintf('%.1f', pst));
end

if isequal(str, 'Start pitch shifter')
    set(handles.pb_pitchShift, 'String', 'Stop pitch shifter', 'ForegroundColor', [1, 0, 0]);
    doPitchShift(pst);
else
    TransShiftMex_PS(2);
    set(handles.pb_pitchShift, 'String', 'Start pitch shifter', 'ForegroundColor', [255 / 255, 177 / 255, 100 / 255]);
end
return



function edit_pitchShift_Callback(hObject, eventdata, handles)
% hObject    handle to edit_pitchShift (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_pitchShift as text
%        str2double(get(hObject,'String')) returns contents of edit_pitchShift as a double


% --- Executes during object creation, after setting all properties.
function edit_pitchShift_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_pitchShift (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in rb_showReadyGo.
function rb_showReadyGo_Callback(hObject, eventdata, handles)
% hObject    handle to rb_showReadyGo (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of rb_showReadyGo
val = get(handles.rb_showReadyGo, 'Value');
load(handles.uiConfigFN);   % gives uiConfig
uiConfig.bShowReadyGo = ~val;
save(handles.uiConfigFN, 'uiConfig');
...
% --- Executes on button press in button_markBad.
function button_markBad_Callback(hObject, eventdata, handles)
% hObject    handle to button_markBad (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if isequal(get(handles.button_markBad, 'String'), 'Mark as bad')
    set(handles.button_markBad, 'String', 'Un-mark as bad');
else
    set(handles.button_markBad, 'String', 'Mark as bad');
end
